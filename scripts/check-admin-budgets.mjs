...
const buildDir = resolve(ROOT, options.buildDir ?? 'admin/.next');
...
if (routes.length < quality.minimum_route_count) {
  failures.push(`chi tim thay ${routes.length} route build, can ${quality.minimum_route_count}`);
}
...
process.stdout.write(
  `\nA5 admin JS budget: ${routes.length} route, lon nhat ${largest?.route ?? '-'} ` +
    `${largest?.gzipKib.toFixed(1) ?? '0.0'}/${quality.javascript_budget_kib} KiB gzip\n`,
);
for (const failure of failures) process.stderr.write(`  - ${failure}\n`);
if (failures.length > 0) process.exitCode = 1;

function measureRoute(entry, files, directory) {
