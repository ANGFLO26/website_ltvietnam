...
Plan này mô tả đầy đủ cách triển khai MVP Website LT Vietnam sau correction FV-01–FV-14 và Round 6B RI-01–RI-03. Nội dung normative nằm trong chính bộ v0.4.1. Lịch sử plan chỉ là provenance, không phải tài liệu bắt buộc để implementer hiểu hoặc thực hiện kế hoạch.
...
Plan không được âm thầm đổi Approved scope, schema hoặc URL. Baseline SQL 001–070 không bị sửa. Mọi implementation schema mới được ghi `IMPLEMENTATION MIGRATION 071+`; Round 6B không tạo SQL.
...
Git, B23, B24 và B25 không chặn Gate A. v0.4.1 hiện chưa đạt Gate A vì chưa có independent final verification PASS và user approval.
...
- **FV-08 / RI-03:** durable `inquiry_outbox_attempts`; commit `started` trước provider call; provider call ngoài DB transaction; result transaction mới; crash/DB-down reconciliation, new attempt per retry và no-blind-resend.
...
- **Gate A blockers:** independent final verification chưa PASS; user chưa phê duyệt. Không còn residual correction nào được chủ ý defer trong nội dung v0.4.1.
