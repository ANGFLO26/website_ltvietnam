...
| 31 | Atomic idempotency D19 | inquiries | P7 | durable replay lookup before CAPTCHA/submission quota; global atomic write | U,DB,API,C | lost-response replay with expired CAPTCHA/exhausted quota; different payload 409 before guards; replay writes nothing; one winner |
| 32 | Outbox worker/reconciliation | worker | P7/P11 | committed attempt-before-send, external provider call, result transaction, reports | U,DB,C,E,SEC,PERF | no call if attempt commit fails; no DB transaction across SMTP; crash/DB-down after accept; new attempt per retry; no-blind-resend |
