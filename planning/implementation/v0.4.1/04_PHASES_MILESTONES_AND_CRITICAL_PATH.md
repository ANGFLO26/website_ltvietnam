...
- SMTP/provider call occurs outside DB transaction. A new result transaction writes `accepted` or `failed` and then coordinates outbox/inquiry status. Ambiguous/crash outcome becomes `started`/`unknown` for reconciliation.
- Minimum attempt states: started, accepted, failed, unknown, confirmed-sent, confirmed-duplicate, confirmed-not-sent. Manual outcome records actor/time/reason. Every retry creates a new attempt number.
...
- **Tests:** seven replay-ordering cases; all prior D19 races/timeouts/legacy cases; attempt commit failure prevents provider call; crash after attempt commit/before call; crash after provider accepted/before attempt update; ambiguous timeout later accepted; DB unavailable after accepted; reaper with started/unknown; retry creates new attempt; manual actor/time/reason; two workers/shutdown/poison/fairness; no PII/body; PG-only Inquiry 202.
- **Acceptance/DoD:** lead-safe replay-before-guards and atomic new write; durable attempt exists before every provider call; no DB transaction spans SMTP network call; lifecycle/reconciliation/no-blind-resend PASS; report contains all required operational states.
