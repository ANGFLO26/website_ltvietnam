...
- Worker attempt state machine/transaction boundaries have a single worker owner; provider adapter cannot bypass committed attempt-start and reconciliation evidence contracts.
...
- Worker health, attempt-before-send lifecycle, reconciliation and manual outcomes: Worker + Ops; DB/concurrency reviewer validates transaction boundaries and no-blind-resend.
