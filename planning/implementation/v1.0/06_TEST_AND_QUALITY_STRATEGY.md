...
- D19 canonicalization từng field, null/missing/empty, Unicode/NFC, line endings, email/phone/UUID/locale, exact accepted P0 DTO names including `source_url`/`privacy_consent`, unknown-field rejection and hash version.
...
- Outbox retry schedule, stable Message-ID, `attempt_state`/`provider_outcome`/`manual_resolution` transitions, system-unknown versus manual-unknown, new attempt number per retry, duplicate-suspected classification và no-blind-resend.
...
- Worker claim/reaper/lease; committed attempt-start before provider call; three-axis attempt transitions; atomic attempt-result + outbox-status + inquiry-email-status transaction; attempt history never overwritten.
...
9. Malformed, empty, whitespace-only, over-length and case-variant keys are rejected with a stable `400` before any durable lookup, and create no Inquiry, Outbox or attempt.
10. The key is accepted from the `Idempotency-Key` header and from `body.request_id`; when both are present the header wins; a header/body mismatch is rejected as `400`.
11. Replayed response is byte-identical to the original across outbox `pending`/`processing`/`sent`/`failed` and inquiry `email_pending`/`email_sent`/`email_failed`, contains no PII field, and is produced without reading any stored response body.
12. Fingerprint v1 uses the exact accepted P0 DTO field names including `source_url` and `privacy_consent`; CI/contract validation fails if the accepted business-field set and canonical schema diverge.
...
9. Manual resolution has actor/time/reason.
...
Each provider call is bracketed by short transactions: claim/commit attempt-start → provider call outside DB transaction → **one single result transaction** for attempt result + `inquiry_outbox.status` + `inquiries.email_status`. The three orthogonal fields are `attempt_state` (started/accepted/failed/unknown/resolved), nullable `provider_outcome` (accepted/rejected/timeout/error), and nullable operator-only `manual_resolution` (confirmed-sent/confirmed-duplicate/confirmed-not-sent/unknown with actor/time/reason). Result-transaction failure leaves the attempt started/unknown for reconciliation and never triggers blind resend. Configurable attempt retention remains at least preliminary 90 days.
