...
| R-09 | Inquiry lost, duplicated or legitimate replay blocked by stale guards | M | H | P7 | Inquiry/DB owner | Durable existing-key resolution before CAPTCHA/submission quota; same replay/different 409; new keys alone run guards; global unique atomic write final arbiter; replay writes nothing; timeout/legacy tests | Committed retry fails on expired CAPTCHA/quota, replay creates rows/attempt, false replay/409, two Inquiry/Outbox rows | PHASE GATE P7 |
...
| R-11 | Media exposed/deleted inconsistently or protected document reaches public root | M | H | P3/P11 | Media/Ops | Separate `public-media/` and `protected-documents/`; proxy read-only public root only; orphan move+cache purge; Semantics A delay/TTL/usage; same-cutoff namespace/permission restore | Guessed protected `/media` URL works, proxy maps volume root, orphan still served, purged URL cached indefinitely, restore changes permissions | PHASE GATE P3/P11 |
...
| R-19 | Upload/content/email injection or storage-boundary traversal | M | H | P3/P6/P7 | Security owner | Magic bytes/storage-class allowlist, sanitization, CRLF, canonical public-root containment, no symlink traversal, deny dotfile/temp/quarantine/direct protected internal route | SVG/MP4 accepted, script executes, unsafe filename/header, `../` or symlink reaches protected namespace | PHASE GATE affected phase |
...
| R-29 | Worker crash/DB outage around provider call causes untraceable duplicate/unknown email | M | H | P7/P11 | Worker/Ops | Commit `started` attempt before send; no call on commit failure; provider call outside DB transaction; orthogonal attempt/provider/manual fields; one result transaction atomically commits attempt + outbox + inquiry email status; preserve provider evidence on manual resolution; new attempt per retry; lease-aware reaper; ≥90d history; no-blind-resend | Provider called without attempt, DB transaction spans SMTP, split two-updated/one-stale result, accepted evidence overwritten, system/manual unknown merged, started/unknown blindly reaped, attempt overwritten | PHASE GATE P7/P11 |
