...
| P7 | SMTP/CAPTCHA/batch/timeout/recipient policy closed; D19 key/replay/stable-result contract ready; committed-attempt-before-send, three-axis attempt model and atomic result-transaction contract ready |
...
| P7 | UUID-v4 key/transport validation and stable no-PII 202 replay; lookup before CAPTCHA/submission quota; lost-response retry bypasses stale guards; atomic new-key write gives one Inquiry/Outbox; attempt-start commits before provider call; no DB transaction spans SMTP; `attempt_state`/`provider_outcome`/`manual_resolution` remain orthogonal; attempt result + outbox status + inquiry email status commit in one result transaction; retry adds attempt; crash/DB-down reconciliation/manual audit/no-blind-resend; PG-only 202 |
...
- Baseline v1.0 standalone, decisions/strategy/tests/risks/phases/gates/RACI complete.
...
Gate A đã PASSED sau Claude independent verification và user approval. Gate B vẫn NOT MET. Kế hoạch được phê duyệt cho implementation nhưng coding không được phép cho tới khi Gate B PASSES; không suy diễn `READY TO CODE`, `IMPLEMENTATION STARTED`, `P0 STARTED` hoặc `GATE B PASSED`.
