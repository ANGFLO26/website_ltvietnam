...
| FV-08 | Medium | **APPLIED** | Attempt-start commits before provider call; network call outside DB transaction; three-axis attempt model; one result transaction for attempt/outbox/inquiry; crash/reaper/new-attempt/manual-audit lifecycle; ≥90d retention/no blind resend | `01` D6/§C.7; `04` P7/P11; `05`; `06`; `07`; `08`; `09` R-29; `10`; `16` | Verify attempt-before-send; three orthogonal fields; atomic three-way result; no transaction across SMTP; manual resolution preserves provider evidence; crash/DB-down/new attempt/no blind resend |
...
## v1.0 Claude delta overlay

| Finding | Severity | v1.0 disposition | Materialization | Validation |
|---|---|---|---|---|
| CF-01 | Medium | **APPLIED** | `01` §B-bis AR-1–AR-4; declared-divergence precedence; `10` provenance reference | Register entries and precedence sentence present |
| CF-02 | Medium | **APPLIED** | `01` §C.1/C.6; `05`; `06` | UUID v4/122-bit/100 chars, byte-exact dual transport and two mandatory tests present |
| CF-03 | Medium | **APPLIED** | `01` §C.4/C.6; `05`; `06` | Stable 202 `{request_id,message}`, no PII/raw body/marker and byte-identical test present |
| CF-04 | Medium | **APPLIED** | `01` §C.7; `04` P7; `05`–`10`; `15`; `16` | Three axes, one result transaction and two DB tests present |
| CF-05 | Low | **APPLIED** | `01` A9 | Inquiry email enum matches Approved CHECK; outbox enum unchanged |
| CF-06 | Low | **APPLIED** | `01` §C.2/C.3 | `source_url`/`privacy_consent`; rejected non-P0 fields excluded |

The FV table is historical disposition evidence. Claude independently verified the source candidate, the user approved promotion, and the current baseline is promoted through `18_V1_0_PROMOTION_RECORD.md`; this table does not assert Gate B PASS.
