...
Round 6B only corrected the three residual contracts below. At that round this file did not approve or promote the candidate. Promotion is now separately recorded in `18_V1_0_PROMOTION_RECORD.md`; this file remains provenance for RI-01–RI-03.
...
| RI-03 Durable attempt lifecycle | HIGH | **APPLIED** | `00`,`01`,`03`,`04`,`05`,`06`,`07`,`08`,`09`,`10`,`15`,`16`, changelog, `17` | **PASS:** committed attempt before send; no call on commit failure; no DB transaction across SMTP; three-axis state model; atomic result transaction; crash/DB-down/reaper/new-attempt/manual-audit/no-blind-resend tests |
...
Every provider call requires a committed `attempt_state='started'` first. Provider I/O occurs outside DB transactions; one subsequent result transaction atomically commits attempt result, outbox status and inquiry email status. `attempt_state`, `provider_outcome` and `manual_resolution` remain orthogonal; manual resolution preserves provider evidence and records actor, time and reason. Each retry adds an attempt. Crash/ambiguous/DB-down outcomes preserve stable Message-ID and enter reconciliation; unknown outcomes are never blindly resent.
...
This author validation was not independent final verification and did not approve the candidate at Round 6B. Claude later verified v0.4.1 independently; current user approval and promotion are recorded separately.
