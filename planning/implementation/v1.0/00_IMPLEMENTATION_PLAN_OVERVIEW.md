...
Plan này mô tả đầy đủ cách triển khai MVP Website LT Vietnam sau correction FV-01–FV-14, Round 6B RI-01–RI-03 và Claude deltas CF-01–CF-06. Nội dung normative nằm trong chính bộ v1.0. Lịch sử plan chỉ là provenance, không phải tài liệu bắt buộc để implementer hiểu hoặc thực hiện kế hoạch.
...
Plan không được âm thầm đổi Approved scope, schema hoặc URL. Các refinement được khai báo minh bạch tại Approved-document reconciliation register trong `01` §B-bis. Baseline SQL 001–070 không bị sửa. Mọi implementation schema mới được ghi `IMPLEMENTATION MIGRATION 071+`; assembly v1.0 không tạo SQL.
...
### Gate A — Plan promotion eligibility — PASSED

- Không Critical; không High chưa xử lý; Medium có disposition và gate rõ.
- v0.4.1 standalone; A1–A25 và D1–D20 đầy đủ.
...
Git, B23, B24 và B25 không chặn Gate A. Claude independent final verification có verdict `PASS WITH MINOR v1.0 EDITS`; user đã phê duyệt promotion ngày 2026-07-25 và CF-01–CF-06 được materialize trong baseline này. **Gate A: PASSED.**

### Gate B — Coding start — NOT MET
...
Pre-P0 Git và các điều kiện môi trường chưa được chứng minh. **Coding: NOT AUTHORIZED UNTIL GATE B PASSES.**

## 5. Các contract correction bắt buộc

- **FV-01 / D19 / RI-01:** durable existing-key replay resolution trước CAPTCHA/new-submission quota; global unique atomic inquiry+outbox write remains final arbiter; response-lost retry không phụ thuộc guard cũ; replay creates no write.
...
- **FV-03 / D20 / RI-02:** Media Semantics A; `/media/*` chỉ map read-only `public-media/`; protected documents/private paths ngoài served root; orphan move+cache invalidation; namespace/permission restore bắt buộc.
...
- **FV-08 / RI-03 / CF-04:** durable `inquiry_outbox_attempts`; commit `attempt_state='started'` trước provider call; provider call ngoài DB transaction; `attempt_state`/`provider_outcome`/`manual_resolution` tách biệt; một result transaction ghi attempt+outbox+inquiry; crash/DB-down reconciliation, new attempt per retry và no-blind-resend.
...
- **Gate A:** PASSED; Critical 0, High 0, sáu Claude deltas đã áp dụng và user approval đã ghi nhận.
...
## 8. Giới hạn và quyền triển khai

Round 8 chỉ assembly/promote tài liệu: không source code, không migration SQL, không Git mutation, không sửa tài liệu Approved và không cleanup/archive/delete/move lịch sử. v1.0 là `APPROVED FOR IMPLEMENTATION — PLANNING COMPLETE`, nhưng không phải `READY TO CODE`: Gate B vẫn NOT MET và implementation chưa bắt đầu.
