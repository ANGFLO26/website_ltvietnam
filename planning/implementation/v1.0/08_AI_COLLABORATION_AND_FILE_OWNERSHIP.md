...
| D19 replay ordering + worker attempt lifecycle | Inquiry/worker owner | Ops/Tech lead | DB+concurrency reviewer | Operations evidence owner |
| Public/protected storage namespace, purge/restore | Media/Ops owner | Ops lead | Security + DB reviewer | Operations evidence owner |
...
- Storage namespace adapter/proxy configuration has a single security owner; changes to public root, protected internal location, symlink policy or permissions require Media+Ops implementation and independent Security review.
- Worker attempt state machine/transaction boundaries have a single worker owner; provider adapter cannot bypass committed attempt-start, the three orthogonal fields (`attempt_state`, `provider_outcome`, `manual_resolution`), the one-result-transaction rule or reconciliation evidence contracts.
...
- Media readiness/purge/namespace/permission consistency: Media + Ops, with Security and DB reviewers.
- Worker health, attempt-before-send lifecycle, reconciliation and manual resolutions: Worker + Ops; DB/concurrency reviewer validates no transaction across provider I/O, atomic three-way result commit, preservation of provider evidence and no-blind-resend.
