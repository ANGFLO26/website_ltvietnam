...
**Input:** P2; B25 closed.  
**Output:** secure upload/processing, StoragePort, MediaUsageService, validators, `/health/ready/media`, public Semantics A lifecycle và physical namespace boundary.

- Enforce magic bytes/MIME, JPG/PNG/WebP/PDF only, no SVG/video, resource limits, EXIF policy and safe storage keys.
- Route validated storage class: public marketing originals/variants vào `public-media/{originals,variants}`; gated documents/PDF vào `protected-documents/`; temp/quarantine vào private non-public location. Không dựa chỉ vào extension.
- Proxy `/media/*` chỉ map read-only tới `public-media/`, không volume root; deny direct protected location, listing, traversal, symlink escape, dotfiles, temp và quarantine.
...
- **Tests:** public image works; guessed `/media/...` for protected PDF=404; direct internal protected location denied; `../` and symlink public→protected denied; dotfile/temp/quarantine denied; MIME spoof/SVG/video/bomb/timeout/duplicate; soft-delete/purge/cache; orphan move makes old served path ineffective under cache policy; backup/restore preserves namespace and permissions.
- **Acceptance/DoD:** media security and namespace isolation PASS; proxy cannot reach volume root/protected documents; Model B media endpoint reflects storage; core API and inquiry acceptance unaffected by storage down; restore namespace/permission scan PASS.
...
- **Tests:** publish/locale per entity, customer visibility, category RESTRICT, external video, gated document download, headers, hidden document denial, guessed public-media URL for protected PDF=404 và Internet access tới internal redirect location bị deny.
- **Acceptance/DoD:** all core entities publish/render; external video safe; protected document nằm ngoài public root và chỉ được Nest-authorized delivery; thin E2E PASS.
...
- Inquiry flow implements D19 ordering: parse/normalize/fingerprint → durable existing-key lookup → same replay hoặc different 409 **trước CAPTCHA/submission quota** → CAPTCHA/rate/external guards chỉ cho new key → atomic unique write remains final arbiter.
- Response-lost retry với expired CAPTCHA hoặc exhausted submission quota vẫn replay; replay DB failure trả retriable safe error và replay path không tạo Inquiry/Outbox/attempt.
- Inquiry winner transaction implements global unique/fingerprint v1 and exactly one outbox; concurrent no-row lookups vẫn hội tụ qua unique conflict/read winner.
...
- Attempt history fields: `id`, `outbox_id`, `attempt_number`, `worker_id`, `stable_message_id`, `provider_message_id`, `provider_response_code`, `provider_response_status`, `started_at`, `accepted_at`, `finished_at`, `attempt_state`, `provider_outcome`, `manual_resolution`, `sanitized_error_code`, `sanitized_error_summary`, `created_at`. Do not overwrite prior attempts.
- Before every provider call: claim job; create/commit a new `started` attempt with attempt number, worker, stable Message-ID and timestamp in a short transaction. Provider must not be called if this commit fails.
- SMTP/provider call occurs outside DB transaction. After the call, **one single result transaction** commits the attempt result, `inquiry_outbox.status` and `inquiries.email_status`; it is distinct from attempt-start and never spans the provider call. Result-transaction failure leaves the attempt `started`/`unknown` for reconciliation and never permits blind resend.
- Three orthogonal axes are mandatory: `attempt_state` = started/accepted/failed/unknown/resolved; nullable `provider_outcome` = accepted/rejected/timeout/error; nullable operator-only `manual_resolution` = confirmed-sent/confirmed-duplicate/confirmed-not-sent/unknown with actor/time/reason. A manual resolution moves state to `resolved` without rewriting provider outcome or timestamps. System-unknown and manual-unknown remain distinguishable. Every retry creates a new attempt number.
...
- Manual resolutions: confirmed-sent, confirmed-duplicate, confirmed-not-sent, unknown. Unknown/duplicate-suspected has **no blind resend**; automatic retry only when previous attempt is known not accepted.
- Lease/heartbeat must keep reaper from taking a legitimately active provider call; reaper encountering started/unknown routes to reconciliation, not resend.
...
- **Tests:** seven replay-ordering cases plus malformed/transport/stable-result cases; all prior D19 races/timeouts/legacy cases; attempt commit failure prevents provider call; crash after attempt commit/before call; crash after provider accepted/before attempt update; ambiguous timeout later accepted; DB unavailable after accepted; reaper with started/unknown; retry creates new attempt; atomic three-way result transaction; manual resolution preserves provider outcome/timestamps and distinguishes two unknowns; two workers/shutdown/poison/fairness; no PII/body; PG-only Inquiry 202.
- **Acceptance/DoD:** lead-safe replay-before-guards and atomic new write; durable attempt exists before every provider call; no DB transaction spans SMTP network call; three-axis lifecycle and single result transaction PASS; reconciliation/no-blind-resend PASS; report distinguishes system and manual unknown.
...
- Run security/regression/load/no-N+1, mixed-version/blue-green, DB+media restore consistency including namespace/permissions, content counts/links/redirects, worker reconciliation/drain.
- Operational health thresholds are tuned and documented; DEGRADED does not remove core API when PG is usable.
- **Acceptance/DoD:** no Critical/High security; budgets/SLO disposition; backup/restore + public/protected namespace/permission scans PASS; durable attempt crash/DB-down-after-accept/no-blind-resend suite PASS; CM4 signed by C7; C5/C9 complete; user go-live approval.
