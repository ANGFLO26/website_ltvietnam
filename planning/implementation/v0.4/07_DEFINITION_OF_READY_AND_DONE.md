...
| P2 | B23+B24 closed |
| P3 | B25 closed; 30-day media purge default accepted as preliminary or replaced by explicit business value |
| P4 | P0 spike PASS; thin routes/E2E defined |
| P5 | product concurrency strategy closed |
| P6A/B | entity and relation contracts frozen |
| P7 | SMTP/CAPTCHA/batch/timeout/recipient policy closed; D19 and attempt history contract ready |
...
| P3 | Upload/media security; Semantics A soft-delete/purge/cache/orphan/restore tests; `/health/ready/media`; storage down does not remove core API |
...
| P6A | Core content publish/download/video validation E2E PASS |
| P6B | Replace-set concurrency/integration PASS; fresh reviewer |
| P7 | D19 mandatory cases; exactly one Inquiry/Outbox; two-worker/drain/reaper; durable attempts/reconciliation/manual outcomes/no-blind-resend; PG-only 202 acceptance |
...
| P11 | Security/load/compatibility/DB+media restore/CM4/reconciliation PASS; C7/C5/C9 sign-offs; user go-live decision |

## 6. Media and rollback Done rules

P3/P11 are not DONE if DB+media restore uses different cutoff or consistency scan is missing. Soft-delete is not proof of file revocation under Semantics A; purge/cache expiry evidence is required. Missing file never triggers automatic DB record deletion.
