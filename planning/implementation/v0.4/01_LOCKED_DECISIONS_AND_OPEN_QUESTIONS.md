...
| D6 | Worker process riêng | Stop-claim, drain, heartbeat, lease, reaper, retry, structured logs; durable attempt history |
...
| D19 | Atomic idempotency contract | Global unique key, versioned durable fingerprint, atomic inquiry+outbox, deterministic replay/409; chi tiết §C |
| D20 | Public media delivery | `/media/*` cho public marketing assets; documents qua Nest; Semantics A public-until-purge; chi tiết §D |
...
### C.4. Atomic transaction và race behavior

1. Validate request/CAPTCHA/rate limit và tính deterministic fingerprint trước transaction.
2. Begin database transaction.
3. Thực hiện atomic insert theo global `idempotency_key`; không có read-before-insert race.
4. Winner tạo Inquiry và đúng một Outbox row trong cùng transaction.
5. Winner commit cả Inquiry + Outbox rồi trả 202/result mới.
6. Loser gặp unique conflict phải chờ outcome transaction của winner theo PostgreSQL unique-conflict behavior, sau đó đọc winner bằng idempotency key.
7. Nếu stored version + fingerprint giống request: trả lại original stable result; không tạo Inquiry/Outbox mới.
8. Nếu version hoặc fingerprint khác: trả 409 `IDEMPOTENCY_KEY_REUSED`; không lộ payload cũ.
9. Atomic repository contract có thể dùng insert-on-conflict-returning hoặc equivalent, nhưng tuyệt đối không có khoảng check rồi insert.
...
- Timeout/network loss sau commit không rõ: client retry cùng key; repository resolve committed winner và trả original result. Nếu chưa thể xác định trong response window, trả trạng thái retriable/có thể query, không tạo key mới.
...
Concurrent same-key/same-fingerprint; same-key/different-fingerprint; exact simultaneous arrival; unique conflict after transaction wait; retry after timeout; rollback before commit; commit succeeded/response lost; legacy NULL row; version mismatch; canonicalization-equivalent inputs; materially different inputs; assert exactly one Inquiry + one Outbox.
...
- DB missing/file present: orphan quarantine, không index/serve mới, delete sau quarantine grace nếu không recover. DB present/file missing: mark BROKEN/DEGRADED, alert, không tự xóa DB. Purged URL trả 404 hoặc 410 theo route policy sau cache expiry/purge.
- Backup/restore dùng cùng snapshot/cutoff cho DB + media; restore chỉ PASS sau existence, checksum, orphan và missing-file scans.
