...
Internet
  → Nginx/Caddy TLS reverse proxy
      ...
      → /media/* public volume delivery
      → Worker process (internal health)
  Nest + Worker → PostgreSQL 16
  Nest/Proxy → persistent media volume through defined adapter/path
...
| `/api/v1/documents/:slug/download` | Nest controlled delivery | Core + route-local media dependency | no-store/policy | media down/missing → stable 503/404, no core removal |
...
| `/media/*` | Proxy→public media volume | route-local media readiness | versioned, max-age preliminary 24h | missing/purged 404 or 410 |
...
- DB missing/file present → orphan quarantine; DB present/file missing → BROKEN/DEGRADED + alert/repair; soft-deleted/file present is expected until purge; purged URL 404/410.
- DB+media restore shares cutoff; existence/checksum/orphan/missing scans must PASS.
...
Proxy normalizes trusted forwarding headers; request ID propagates Proxy→Next→Nest→worker. Internal health endpoints are access-controlled. Public media does not expose directory/host/path internals. Logs contain no full PII or content body.
