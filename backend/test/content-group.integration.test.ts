...
run('Nhom noi dung: posts, projects, documents, customers', () => {
  ...
  beforeAll(async () => {
    ...
    const pdf = await daos.media.insert({
      fileName: `${tag}-cat.pdf`,
      originalName: `${tag} catalogue.pdf`,
      storageClass: 'protected',
      storagePath: `protected-documents/${tag}/cat.pdf`,
  ...
  afterAll(async () => {
    ...
    await pool.query(
      `DELETE FROM ltv.projects WHERE customer_id IN
      (SELECT id FROM ltv.customers WHERE name LIKE $1)`,
      [`${tag}%`],
    );
    await pool.query(
  ...
  it('danh muc bai viet la cay va co slug KHONG theo locale', async () => {
    ...
    expect((await daos.postCategories.findSubtreeIds(id['cat']!)).sort()).toEqual(
      [id['cat'], con.id].sort(),
    );
  ...
  it('bai viet: bon nhom quan he thay ca tap trong MOT lan goi', async () => {
    ...
    expect((await daos.posts.findLinks(post.id)).brandIds.sort()).toEqual(
      [brandA.id, brandB.id].sort(),
    );
  ...
  const makeProject = async (
    ...
    const cus = await daos.customers.insert({
      name: `${tag} Nha may loc dau Dung Quat`,
      industryId: id['ind']!,
      logoId: id['logo']!,
    });
  ...
  it('public  →  ten hien thi neu co, khong thi ten that', async () => {
    ...
    expect(await daos.projects.resolvePublicCustomerName(pj.id, 'vi')).toBe(
      'Mot nha may loc dau mien Trung',
    );
  ...
  it('tep bi xoa mem thi tai lieu khong tai duoc nua', async () => {
    const m = await daos.media.insert({
      fileName: `${tag}-tam.pdf`,
      originalName: `${tag} tam.pdf`,
      storageClass: 'protected',
      storagePath: `protected-documents/${tag}/tam.pdf`,
      mimeType: 'application/pdf',
      fileExtension: 'pdf',
      fileSize: 10,
    });
  ...
  it('bo dem tai xuong tang tai cho', async () => {
    ...
    await Promise.all(
      Array.from({ length: 8 }, () => daos.documents.recordDownload(d.id, new Date())),
    );
