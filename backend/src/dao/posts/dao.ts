...
export class KyselyPostDao extends BaseDao implements PostDao {
  ...
  async list(filter: PostFilter, page?: Partial<Page>): Promise<Paged<Post>> {
    ...
    if (filter.categoryId) {
      ...
      cq = cq.where('category_id', '=', filter.categoryId);
    }
  ...
  async upsertTranslation(id: string, input: UpsertPostTranslationInput): Promise<PostTranslation> {
    const values = {
      post_id: id,
      locale: input.locale,
      title: input.title,
  ...
  async replaceLinks(id: string, links: PostLinks): Promise<void> {
    if (links.productIds !== undefined) {
      ...
      if (links.productIds.length > 0) {
        await this.db
          .insertInto('post_products')
          .values(links.productIds.map((x) => ({ post_id: id, product_id: x })))
          .execute();
      }
    }
    if (links.serviceIds !== undefined) {
      ...
      if (links.serviceIds.length > 0) {
        await this.db
          .insertInto('post_services')
          .values(links.serviceIds.map((x) => ({ post_id: id, service_id: x })))
          .execute();
      }
    }
    if (links.projectIds !== undefined) {
      ...
      if (links.projectIds.length > 0) {
        await this.db
          .insertInto('post_projects')
          .values(links.projectIds.map((x) => ({ post_id: id, project_id: x })))
          .execute();
      }
    }
    if (links.brandIds !== undefined) {
      ...
      if (links.brandIds.length > 0) {
        await this.db
          .insertInto('post_brands')
          .values(links.brandIds.map((x) => ({ post_id: id, brand_id: x })))
          .execute();
      }
    }
  }
  ...
  async findLinks(id: string): Promise<Required<PostLinks>> {
    const [pr, sv, pj, br] = [
      await this.db
        .selectFrom('post_products')
        .select('product_id')
        .where('post_id', '=', id)
        .execute(),
      await this.db
        .selectFrom('post_services')
        .select('service_id')
        .where('post_id', '=', id)
        .execute(),
      await this.db
        .selectFrom('post_projects')
        .select('project_id')
        .where('post_id', '=', id)
        .execute(),
      await this.db
        .selectFrom('post_brands')
        .select('brand_id')
        .where('post_id', '=', id)
        .execute(),
    ];
