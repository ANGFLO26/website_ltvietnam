...
export class KyselyDocumentDao extends BaseDao implements DocumentDao {
  ...
  async list(filter: DocumentFilter, page?: Partial<Page>): Promise<Paged<AppDocument>> {
    ...
    if (filter.visibility) {
      ...
      cq = cq.where('visibility', '=', filter.visibility);
    }
    if (filter.documentType) {
      q = q.where('document_type', '=', filter.documentType);
      cq = cq.where('document_type', '=', filter.documentType);
    }
    if (filter.language) {
      q = q.where('language', '=', filter.language);
      cq = cq.where('language', '=', filter.language);
    }
    if (filter.search) {
      const needle = `%${escapeLike(filter.search)}%`;
      q = q.where((eb) =>
        eb.or([eb('title', 'ilike', needle), eb('description', 'ilike', needle)]),
      );
      cq = cq.where((eb) =>
        eb.or([eb('title', 'ilike', needle), eb('description', 'ilike', needle)]),
      );
  ...
  async replaceLinks(id: string, links: DocumentLinks): Promise<void> {
    if (links.productIds !== undefined) {
      ...
      if (links.productIds.length > 0) {
        await this.db
          .insertInto('document_products')
          .values(
            links.productIds.map((x, i) => ({ document_id: id, product_id: x, display_order: i })),
          )
          .execute();
      }
    }
    if (links.brandIds !== undefined) {
      ...
      if (links.brandIds.length > 0) {
        await this.db
          .insertInto('document_brands')
          .values(links.brandIds.map((x) => ({ document_id: id, brand_id: x })))
          .execute();
      }
    }
    if (links.serviceIds !== undefined) {
      ...
      if (links.serviceIds.length > 0) {
        await this.db
          .insertInto('document_services')
          .values(links.serviceIds.map((x) => ({ document_id: id, service_id: x })))
          .execute();
      }
    }
    if (links.postIds !== undefined) {
      ...
      if (links.postIds.length > 0) {
        await this.db
          .insertInto('document_posts')
          .values(links.postIds.map((x) => ({ document_id: id, post_id: x })))
          .execute();
      }
    }
  }
  ...
  async findLinks(id: string): Promise<Required<DocumentLinks>> {
    const pr = await this.db
      .selectFrom('document_products')
      .select('product_id')
      .where('document_id', '=', id)
      .execute();
    const br = await this.db
      .selectFrom('document_brands')
      .select('brand_id')
      .where('document_id', '=', id)
      .execute();
    const sv = await this.db
      .selectFrom('document_services')
      .select('service_id')
      .where('document_id', '=', id)
      .execute();
    const po = await this.db
      .selectFrom('document_posts')
      .select('post_id')
      .where('document_id', '=', id)
      .execute();
    return {
